    ...
    for fname in os.listdir(docset_path):
        if docset_id in fname:
            model_paths = [os.path.join(docset_path, fname, length)
                           for length in [u"50", u"100", u"200", u"400"]]
...
def generate_summaries(systems, docsets):
    rouge = sumpy.rouge.ROUGE(max_ngrams=2, show_per_model_results=False)
    results = []
...
def main(duc_dir):
    ...
    centroid = lambda x: sumpy.centroid
    print generate_summaries([("lede", lede), ("centroid", centroid)], docsets)
